...
*/	
	...
	//================
		...
		autoSave.close();
		delete autoSave;
		if (code == "") code = "/*0.1\nScript créé avec Duduf Actions for After Effects version " + version + "\nhttp://www.duduf.net\n*/\n\n\n" ;
...
function save() {
                    var okToGo = false;
                    while(!okToGo) {                    
                            var save = prompt ("Nom ?","Nom du script","Nom du script");
                            if (save != null) {
                                  ...
                                  else okToGo = confirm("Ce script existe déjà, voulez-vous le remplacer ?",true,"Remplacer l'ancien script");
                                  }
                            saveFile.open("w");
                            saveFile.write(code);
                            saveFile.close();
...
//===================
        ...
        boutonSave.helpTip = "Sauvegarder le Script";
        var boutonOpen = palette.add("button",[90,274,110,294],"O");
        boutonOpen.onClick = function () { 
                	var open = File.openDialog ("Ouvrir le script");
                    if (open != null) {
                    open.open("r");
                    code = open.read();
                    open.close();
        ...
        boutonOpen.helpTip = "Ouvrir un Script";
		...
		var boutonDossier = fenetreOptions.add("button",[2,2,120,20],"Dossier des Scripts :");
		boutonDossier.onClick = function () {
			var dossierUser = Folder.selectDialog("Choisissez le dossier contenant les scripts.");
        ...
        //=====================
		var fenetrelib = new Window("palette","Bibliothèque d'Actions");
		...
		// calque / créer
		var calqueCreer = tree.add("node","Calque / Créer");
		// addSolid
		var addSolid = calqueCreer.add("item","Solide");
        ...
        // addText
        var addText = calqueCreer.add("item","Texte");
		addText.script = new File(dossier + "addText.jsx");
        // addAdjustmentLayer
        var addAdjustmentLayer = calqueCreer.add("item","Calque d'effets");
		addAdjustmentLayer.script = new File(dossier + "addAdjustmentLayer.jsx");
         //ikControl
		var ikControl2 = calqueCreer.add("item","Controleur");
		ikControl2.script = new File(dossier + "ikControl.jsx");
        
        // calque / sélection
        var calqueSelect = tree.add("node","Calque / Sélectionner");
        // selectUp
        var selectUp = calqueSelect.add("item","Au dessus");
        selectUp.script = new File(dossier + "selectUp.jsx");
        // selectUpKeep
        var selectUpKeep = calqueSelect.add("item","Garder et au dessus");
        selectUpKeep.script = new File(dossier + "selectUpKeep.jsx");
        // selectDown
        var selectDown = calqueSelect.add("item","En dessous");
        selectDown.script = new File(dossier + "selectDown.jsx");
        // selectDownKeep
        var selectDownKeep = calqueSelect.add("item","Garder et en dessous");
        selectDownKeep.script = new File(dossier + "selectDownKeep.jsx");
        // selectAll
        var selectAll = calqueSelect.add("item","Tout");
        selectAll.script = new File(dossier + "selectAll.jsx");
        // selectNone
        var selectNone = calqueSelect.add("item","Aucun");
        selectNone.script = new File(dossier + "selectNone.jsx");
        // selectInvert
        var selectInvert = calqueSelect.add("item","Inverser");
        selectInvert.script = new File(dossier + "selectInvert.jsx");
        // selectIndex
        var selectIndex = calqueSelect.add("item","Par index");
        selectIndex.script = new File(dossier + "selectIndex.jsx");
        // selectName
        var selectName = calqueSelect.add("item","Par nom");
        selectName.script = new File(dossier + "selectName.jsx");
        
        // calque / manipuler
        var calqueManip = tree.add("node","Calque / Manipuler");
        // layerDuplicate
        var layerDuplicate = calqueManip.add("item","Dupliquer");
        layerDuplicate.script = new File(dossier + "layerDuplicate.jsx");
        // layerAnchorPoint
        var layerAnchorPoint = calqueManip.add("item","Point d'ancrage");
        ...
        // layerScale
        var layerScale = calqueManip.add("item","Echelle");
        layerScale.script = new File(dossier + "layerScale.jsx");
        //layerOpacity
        var layerOpacity = calqueManip.add("item","Opacité");
        layerOpacity.script = new File(dossier + "layerOpacity.jsx");
        // layerAlpha
        var layerAlpha = calqueManip.add("item","Mode de fusion et Alpha");
        layerAlpha.script = new File(dossier + "layerAlpha.jsx");
        // layerAttributes
        var layerAttributes = calqueManip.add("item","Attributs");
        layerAttributes.script = new File(dossier + "layerAttributes.jsx");
        // layerRename
        var layerRename = calqueManip.add("item","Renommer");
        layerRename.script = new File(dossier + "layerRename.jsx");
        // layerPrecomp
        var layerPrecomp = calqueManip.add("item","Précomposer");
        layerPrecomp.script = new File(dossier + "layerPrecomp.jsx");
        //layerMoveUp
        var layerMoveUp = calqueManip.add("item","Monter");
        layerMoveUp.script = new File(dossier + "layerMoveUp.jsx");
        //layerMoveDown
         var layerMoveDown = calqueManip.add("item","Descendre");
        ...
        // annulation
        var annulation = tree.add("node","Annulation");
        // beginUndoGroup
        var beginUndoGroup = annulation.add("item","Début de groupe");
        beginUndoGroup.script = new File(dossier + "beginUndoGroup.jsx");
        // endUndoGroup
        var endUndoGroup = annulation.add("item","Fin de groupe");
        endUndoGroup.script = new File(dossier + "endUndoGroup.jsx");
        
        // scripts
        var scripts = tree.add("node","Scripts");
        // exécuter un script
        var evalScript = scripts.add("item","Exécuter un script");
